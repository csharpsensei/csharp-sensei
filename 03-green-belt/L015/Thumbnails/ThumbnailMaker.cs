using InterfaceSegregation.Storage;

namespace InterfaceSegregation.Thumbnails;

/// <summary>
/// Read the constructor as documentation, because that is what it now is:
/// this class reads files. It does not write and it does not delete, and you
/// did not have to open the body to know that.
/// </summary>
public sealed class ThumbnailMaker
{
    private readonly IReadFiles _files;

    public ThumbnailMaker(IReadFiles files) => _files = files;

    public string Shrink(string name)
    {
        byte[] source = _files.Read(name);
        byte[] small = new byte[source.Length / 4];
        return $"{name} {source.Length} bytes -> {small.Length} bytes";
    }
}
