...
No .NET SDK was reachable in the generation sandbox (network is allowlisted
and the dotnet-install download was blocked), so `dotnet build` has not been
run against this project. Every override's body is written to match its own
narration and the console-output stills; `dotnet run` settles all of it in
seconds once a real SDK is available. Same standing caveat as V009 and V010.
